...
ViewCurve::ViewCurve(QWidget *w_parent) : QWidget(w_parent)
{
  ...
  time_cursor_pen = new QPen(Qt::SolidPattern, 0, Qt::SolidLine, Qt::SquareCap, Qt::BevelJoin);

  score_pen = new QPen(Qt::SolidPattern, 0, Qt::SolidLine, Qt::SquareCap, Qt::BevelJoin);
  ...
  time_cursor_color = Qt::red;

  rem_color = Qt::red;
  nrem_color = Qt::blue;
  wake_color = Qt::darkGray;
...
void ViewCurve::mousePressEvent(QMouseEvent *press_event)
{
  ...
  if(press_event->button()==Qt::LeftButton)
  {
	...
	if(QApplication::keyboardModifiers() == Qt::ControlModifier)
	{
		long long time = ((long long)((((double)m_x) / w) * mainwindow->pagetime))
			+ mainwindow->edfheaderlist[mainwindow->sel_viewtime]->viewtime
			+ mainwindow->edfheaderlist[mainwindow->sel_viewtime]->starttime_offset;
		vector<SPW>::iterator it = mainwindow->scoring.vPW.begin();
		for(;it != mainwindow->scoring.vPW.end();it++)
		{
			if(time > it->start && time < it->end) { mainwindow->scoring.vPW.erase(it); break; }
		}
  ...
  if(press_event->button()==Qt::RightButton)
  {
	...
	if(just_set_cursor && QApplication::keyboardModifiers() == Qt::ControlModifier)
	{
		time_cursor_moving = 1;
		time_cursor_x = m_x;
		long long time = ((long long)((((double)m_x) / w) * mainwindow->pagetime))
			+ mainwindow->edfheaderlist[mainwindow->sel_viewtime]->viewtime
			+ mainwindow->edfheaderlist[mainwindow->sel_viewtime]->starttime_offset;
		time_cursor_set = 1;
		if(!PWannotating) { 
			current_sPW.start = time;
			PWannotating = true;
			mainwindow->scoring.log("pw push");
		}
	}
  }
}
...
void ViewCurve::mouseReleaseEvent(QMouseEvent *release_event)
{
  ...
  if(time_cursor_moving == 1) time_cursor_moving = 0;
  ...
  {
	if(PWannotating && QApplication::keyboardModifiers() == Qt::ControlModifier) { 
		long long time = ((long long)((((double)m_x) / w) * mainwindow->pagetime))
			+ mainwindow->edfheaderlist[mainwindow->sel_viewtime]->viewtime
			+ mainwindow->edfheaderlist[mainwindow->sel_viewtime]->starttime_offset;
		current_sPW.end = time;
		/*if(current_sPW.end < current_sPW.start)
		{
			long long tmp = current_sPW.end; 
			current_sPW.end = current_sPW.start;
			current_sPW.start = tmp;
		}*/
		mainwindow->scoring.vPW.push_back(current_sPW);
		current_sPW.reset();
		PWannotating = false;
		mainwindow->scoring.log("pw release");
		update();
	}
...
void ViewCurve::mouseMoveEvent(QMouseEvent *move_event)
{
  ...
  if(time_cursor_moving == 1)
  {
	  time_cursor_x = mouse_x;
	  time_cursor_time = ((long long)((((double)mouse_x) / w) * mainwindow->pagetime))
		  + mainwindow->edfheaderlist[mainwindow->sel_viewtime]->viewtime
		  + mainwindow->edfheaderlist[mainwindow->sel_viewtime]->starttime_offset;
	  update();
  }
...
void ViewCurve::drawCurve_stage_2(QPainter *painter, int w_width, int w_height, int print_linewidth)
{
  ...
  // ps. sizes
  int font_size = 18;  // for scoring 
  int stage_line_width = 2;
  bool short_scoring_names = false;
  int d_sleep_rectangle = 10;
  int epoch = mainwindow->epoch_duration;
  bool small_color_string = false;

  if(m_pagetime < 6*epoch)
  {
	small_color_string = true;
  }

  if(m_pagetime > 10*epoch)
  {
	  font_size = 8;
	  short_scoring_names = true;
	  stage_line_width = 0;
	  d_sleep_rectangle = 0; 
	  if(m_pagetime > 28*3*epoch)
		  font_size = 5;
  }
  else if(m_pagetime > 3*epoch)
  {
	  font_size = 16;
	  stage_line_width = 1;
	  d_sleep_rectangle = 7;
  ...
  // ps. try plot PW
	...
	{
		SPW spw= *it;
		long long xt = spw.start - mainwindow->edfheaderlist[mainwindow->sel_viewtime]->viewtime -  
			mainwindow->edfheaderlist[mainwindow->sel_viewtime]->starttime_offset;
		int marker_x = (int)((((double)w) / mainwindow->pagetime) * xt);
		long long wt = spw.end - spw.start;
		if((xt + wt) < 0 && xt > w) continue;
		int marker_w = (int)((((double)w) / mainwindow->pagetime) * wt);	  
		painter->fillRect(marker_x, 0,marker_w, h, QBrush(QColor(255, 153, 102, 128)));
	}
  ...
  if(mainwindow->show_annot_markers)
  {
    ...
    for(i=0; i<mainwindow->files_open; i++)
    {
      ...
      j = 0;
	  int epochDur = mainwindow->epoch_duration;
      while(annot!=NULL)
      {
        l_tmp = annot->onset - mainwindow->edfheaderlist[i]->starttime_offset;

        if((l_tmp + epochDur*TIME_DIMENSION*0.5 > (mainwindow->edfheaderlist[i]->viewtime - TIME_DIMENSION)) && (!annot->hided))
        {
		  ...
		  if(l_tmp + epochDur*TIME_DIMENSION*0.5 > (mainwindow->edfheaderlist[i]->viewtime + mainwindow->pagetime))
		  {
			  break;
		  }
          if(!annot_marker_moving)
	...
	// scoring
	if(time_cursor_set == 1)
	{
		time_cursor_pen->setColor(time_cursor_color);
		painter->setPen(*time_cursor_pen);
		painter->drawLine(time_cursor_x, 0, time_cursor_x, h);
		//mainwindow->edfheaderlist[i]->viewtime + mainwindow->edfheaderlist[i]->starttime_offset + mainwindow->pagetime)
	}

	// s. time to show sleep stages
	
	
	int text_free_zone = 200;
	int text_shift = 0; //40
	if(short_scoring_names) text_shift = 5;
	int count = active_markers->count;
	struct annotationblock **list = active_markers->list;
	painter->setPen(*score_pen);
	const QFont oldf = painter->font(); 
	int style = short_scoring_names? QFont::Bold : QFont::Normal;
	QFont f ("Verdana", font_size, style);
	f.setUnderline(false);
	painter->setFont (f);
	current_annotation = NULL;
	bool current_annotation_set = false;
	
	if(count > 0)
	{
		int text_x = 0, text_y = 40;
		int rct_x = 0, rct_h = 0, rct_y = 0, rct_w = 0;
		char buf[256];
		char* text = NULL;
		int current_annotationI = (int)((count-1)/2);  
		current_annotation = list[current_annotationI]; // select epoch in the middle
		//if(count == 1) current_annotation = list[0];
		current_annotation_set = true;
		for(int i = 0; i <= count; i++)
		{
			...
			painter->setFont (f);
			if(i == 0){
			 if( list[i]->former_annotation != NULL &&
				list[i]->x_pos > text_free_zone)
				{
					text_x = list[i]->x_pos/2 - text_shift;
					text = list[i]->former_annotation->annotation;

					rct_w = list[i]->x_pos - 2*d_sleep_rectangle;
					rct_x = d_sleep_rectangle;

					//current_annotation = list[i]->former_annotation; 
				}
			 else continue;
			}
			if(i > 0 && i <= count-1)
			{
				text_x = 0.5*(list[i]->x_pos + list[i-1]->x_pos) - text_shift;
				text = list[i-1]->annotation;

				rct_w = list[i]->x_pos - list[i-1]->x_pos - 2*d_sleep_rectangle;
				rct_x = list[i-1]->x_pos + d_sleep_rectangle;

			}
			if(i == count)
			{
				if(list[i-1]->x_pos < w - text_free_zone &&
					list[i-1]->next_annotation != NULL)
				{
					text_x = 0.5*(list[i-1]->x_pos + w) - text_shift;
					text = list[i-1]->annotation;

					rct_w = w - list[i-1]->x_pos - 2*d_sleep_rectangle;
					rct_x = list[i-1]->x_pos + d_sleep_rectangle;
				}
				else continue;
			}
			if(text == NULL) continue;
			if (i == current_annotationI + 1) { f.setUnderline(true);  f.setBold(true); painter->setFont (f);  }

			// color						
			char cbuf[1024];
			if(text[strlen(text) - 1] == '*') strncpy(cbuf, text, strlen(text) - 1); // crop expert thing
			else strcpy(cbuf, text);
			
			QColor color;
			if(strstr(cbuf, rem_string) != NULL)
				color = Qt::blue;
			if(strstr(cbuf, sw_string) != NULL || strstr(text, "NREM2") != 0)
				color = Qt::green;
			if(strstr(cbuf, wake_string) != NULL)
				color = Qt::gray;
			if(strstr(cbuf, "??") != NULL)
				color = Qt::red;
			
			//handle short and draw
			if(short_scoring_names)
			{
				if(strcmp(text, na_string) != 0)
				{ buf[0] = text[0]; buf[1] = 0; }
				else
					buf[0] = 0;
				text = buf;
			}
			...
			score_pen->setColor(color);
			painter->setPen(*score_pen);

			rct_y = 50;
			rct_h = 20;

			if(small_color_string)
			{
				rct_w = 1.5*d_sleep_rectangle;
				rct_y = text_y-1.5*d_sleep_rectangle;
				rct_h = 1.5*d_sleep_rectangle;
				rct_x = text_x - 3*d_sleep_rectangle;

			}
			
			painter->drawRect(rct_x, rct_y, rct_w, rct_h);
			
			
			score_pen->setColor(Qt::black);
...
//        i, (int)signalcomp, (int)screensamples);
    ...
    for(s=signalcomp->sample_start; s<signalcomp->samples_on_screen; s++)
    {
      ...
      else
      {
		...
		if(fabs(dig_value) > signalcomp->stat_max_value) { signalcomp->stat_max_value = fabs(dig_value); }
        signalcomp->stat_cnt++;
...
void ViewCurve::exec_sidemenu(int signal_nr_intern)
{
  ...
  sidemenu->setMinimumSize(QSize(190, 435));
  sidemenu->setMaximumSize(QSize(190, 435));
  ...
  QObject::connect(sidemenuButton13,  SIGNAL(clicked()),                sidemenu, SLOT(close()));
...
// scoring
bool ViewCurve::isAnnotationScoring(struct annotationblock *annot) // is annotation is sleep stage (true) or something else
{
	if(strcmp(annot->annotation, sw_string)==0 || 
		strcmp(annot->annotation, wake_string)==0 ||
		strcmp(annot->annotation, rem_string)==0)
	{
		return true;
	}
	return false;
