...
#include <numeric>
#include <algorithm>
#include "paraslonic/qcustomplot.h"
...
UI_Mainwindow::UI_Mainwindow()
{
	// scoring
	noPageShift = false;
	epoch_duration = 20; // move to config file!
	motion_wake_threshold = 1;
	epoch_count = 0;
	current_scoring_i = -1;
	motion_signal = -1;
	sum_wake_mot_mrs = 0;
	...
	selected_signal = -1;
	expertMode = false;
	spectrum_max_freq = 50; // move 2 settings
	markPWmode = false; deletePWmode = false;
	//
...
// do not use this code, it will be removed in future
  ...
  page_5->setShortcut(QKeySequence("Ctrl+0"));
  ...
  page_20->setShortcut(QKeySequence("Ctrl+1"));
  ...
  page_div2 = new QAction("Timescale / 3", this);
  ...
  page_mult2 = new QAction("Timescale x 3", this);
...
//   menubar->addMenu(patternmenu);
  ...
  scoremenu->setTitle("S&coring");
  //scoremenu->addAction("Create epoch file", this, SLOT(create_epoch_file()));
  scoremenu->addAction("Split to epochs", this, SLOT(set_epoch_markers()));
  scoremenu->addAction("Save scoring", this, SLOT(save_scoring()));
  scoremenu->addAction("Delete scoring", this, SLOT(delete_scoring()));
  //scoremenu->addAction("Load scoring", this, SLOT(load_scoring()));
  scoremenu->addAction("Report", this, SLOT(report()));
  scoremenu->addAction("Report motion", this, SLOT(reportMotion()));
  scoremenu->addAction("Save spectrums", this, SLOT(save_spectrum()));
  scoremenu->addAction("Expert mode", this, SLOT(change_expert_mode()));
  scoremenu->addAction("Expertize current scoring", this, SLOT(experize_epochs()));
  scoremenu->addAction("Start Auto Scoring", this, SLOT(autoScoring()));
  scoremenu->addAction("Auto score outliers", this, SLOT(autoScoreOutliers()));

  scoremenu->addAction("REM", this, SLOT(score_rem()), QKeySequence(Qt::Key_5));
  scoremenu->addAction("NREM", this, SLOT(score_nrem()), QKeySequence(Qt::Key_2));
  scoremenu->addAction("Wake", this, SLOT(score_wake()), QKeySequence(Qt::Key_0));
  scoremenu->addAction("na", this, SLOT(score_na()), QKeySequence(Qt::Key_7));
  scoremenu->addAction("outlier", this, SLOT(score_out()), QKeySequence(Qt::Key_8));
  //scoremenu->addAction("na", this, SLOT(score_question()), QKeySequence(Qt::Key_Question));
  scoremenu->addSeparator();
  scoremenu->addAction("Select motion signal", this, SLOT(select_motion_signal()));
  scoremenu->addAction("Score by motion", this, SLOT(score_by_motion()));
  scoremenu->addAction("Convert motion txt to edf", this, SLOT(convert_motion_signal()));
  
  // scoring
  scoremenu->addSeparator();
  scoremenu->addAction("toggle Peak-Wave activity", this, SLOT(score_tag_pw()), QKeySequence(Qt::Key_9));

  scoremenu->addAction("Auto score Peak-Wave activity", this, SLOT(autoPWScoring()));
  scoremenu->addAction("Go next Peak-Wave activity", this, SLOT(goNextPW()));
  scoremenu->addAction("Go next outlier", this, SLOT(goNextOut()));

  // next 
  QAction *nextREM = new QAction("REM", this);  
  QAction *nextSW = new QAction("SW", this);  
  QAction *nextWake = new QAction("Wake", this);  
  QAction *nextNA = new QAction("na", this);    
  signalMapper->setMapping(nextREM, s_REM);
  signalMapper->setMapping(nextSW, s_SW);
  signalMapper->setMapping(nextWake, s_Wake);
  signalMapper->setMapping(nextNA, s_NA);
  
  connect(nextREM, SIGNAL(triggered()), signalMapper, SLOT(map()) );
  connect(nextSW, SIGNAL(triggered()), signalMapper, SLOT(map()) );
  connect(nextWake, SIGNAL(triggered()), signalMapper, SLOT(map()) );
  connect(nextNA, SIGNAL(triggered()), signalMapper, SLOT(map()) );

  scoremenu->addSeparator();
  QMenu* nextmenu = new QMenu(this);
  nextmenu->setTitle("Next epoch with");
  nextmenu->addAction(nextREM);
  nextmenu->addAction(nextSW);
  nextmenu->addAction(nextNA);
  nextmenu->addAction(nextWake);
  scoremenu->addMenu(nextmenu);
  connect( signalMapper, SIGNAL(mapped(int)), SLOT(goNextScoring(int)) );
  // pw stat
  scoring.addNewTag(pw_string); // peak wave activity
...
void UI_Mainwindow::shift_page_one_epoch(int dir)
{
	int i;
	...
	if((viewtime_sync==VIEWTIME_SYNCED_OFFSET)||(viewtime_sync==VIEWTIME_SYNCED_ABSOLUT)||(viewtime_sync==VIEWTIME_USER_DEF_SYNCED))
	{
		for(i=0; i<files_open; i++)
		{
			edfheaderlist[i]->viewtime += epoch_duration*TIME_DIMENSION*dir;
		}
	}

	if(viewtime_sync==VIEWTIME_UNSYNCED)
	{
		edfheaderlist[sel_viewtime]->viewtime += epoch_duration*TIME_DIMENSION*dir;
	}
...
void UI_Mainwindow::open_new_file()
{
  ...
  cmdlineargument = 0;
  // scoring
  if(files_open == 1) load_scoring();
  // expert mode
  change_expert_mode(true); // force expert
  // select motion signal
  for(i=0; i<signalcomps; i++)
  {
		if(motion_signal > 0) continue;
		if(strcmp(signalcomp[i]->signallabel, "motion") == 0 ||
			strstr(signalcomp[i]->signallabel, "Accelerometer") != NULL)
		{
			motion_signal = i;
		}
  }	
...
void UI_Mainwindow::set_page_div2()
{
  ...
  if((viewtime_sync==VIEWTIME_SYNCED_OFFSET)||(viewtime_sync==VIEWTIME_SYNCED_ABSOLUT)||(viewtime_sync==VIEWTIME_USER_DEF_SYNCED))
  {
    for(i=0; i<files_open; i++)
    {
      edfheaderlist[i]->viewtime += (pagetime / 3);
    }
  }

  if(viewtime_sync==VIEWTIME_UNSYNCED)
  {
    edfheaderlist[sel_viewtime]->viewtime += (pagetime / 3);
  }
...
{
#ifdef Q_WS_X11
  QDesktopServices::openUrl(QUrl("file:///usr/share/doc/edfbrowser/EDFbrowser%20manual.html"));
#endif
...
void UI_Mainwindow::autoPWScoring()
{
	if(!spectrumdock || !spectrumdock->dock->isVisible()) 
	{
		QMessageBox msgBox;
		msgBox.setText("Open the spectrum window first");
		msgBox.exec();
		return;
	}
	// do 
	int currEpoch = currentEpoch();
	get_current_spectrum(freqstep);
	int minHi = 12; // hz - left border of. 2do: Move 2 config
	int maxHi = 35;
	double medianLevel = 0;
	vector<double> levels; 
	int minHi_i = minHi/freqstep;
	int maxHi_i = maxHi/freqstep;
	// hours
	bool ok;
	int hours = QInputDialog::getInt(this, tr("Auto scoring"),
		tr("Hours (0 - all):"), hours, 0, 1000, 1, &ok);
	int max_epoch;
	if(hours == 0) max_epoch = epoch_count-1;
	else
	{
		max_epoch = currEpoch + hours*3600/epoch_duration;
		if (max_epoch > epoch_count - 1 ) max_epoch = epoch_count - 1;
	}
	int epochs = max_epoch - currEpoch;
	// sensetivity
	double sensetivityFactor = 2.5;
	sensetivityFactor = QInputDialog::getDouble(this, tr("sensetivity"),
		tr("sensetivity (~2-3):"), sensetivityFactor, 0, 20, 1, &ok);
	
	// calc median 
	levels.reserve(epochs); // epoch 2 calc
	for(int i = currEpoch; i < max_epoch; i++)
	{
		if(scoring.outliers[i]) continue;
		jump_epoch(i);
		QApplication::processEvents();
		TStage stage; STag tag;
		epoch_stat stat = getEpochStat();
		vector<double> spectrum = stat.spectrum;
		double maxHiLevel = 0;
		for(int j = minHi_i; j < maxHi_i; j++){ 
			double val = spectrum[j];
			if(val > maxHiLevel) maxHiLevel = val;
		}
		double maxLevel = 0;
		for(int j = 0; j < maxHi_i; j++){ 
			double val = spectrum[j];
			if(val > maxLevel) maxLevel = val;
		}
		double sum = std::accumulate(spectrum.begin(), spectrum.end(), 0);
		levels.push_back(maxHiLevel/maxLevel);
	}
	sort(levels.begin(), levels.end());
	medianLevel = levels[(levels.size()/2)];
	scoring.log("median hi freq level", medianLevel);
	// score pw
	int scored = 0;
	scoring.log("curr epoch", currEpoch);
	scoring.log("max epoch", max_epoch);
	for(int i = currEpoch; i < max_epoch; i++)
	{
		if(scoring.outliers[i]) continue;
		jump_epoch(i);
		...
		if((maxHiLevel/maxLevel) > medianLevel*sensetivityFactor) 
		{
			string name(pw_string);
			scoring.switchTag(i,name, true);
			strcpy(maincurve->current_annotation->annotation, scoring.epochName(i).c_str());
			scored++;
		}
	}
	QMessageBox msgBox;
...
void UI_Mainwindow::goNextPW()
{
	int i = currentEpoch()+1;
	for(;i< epoch_count; i++) 
		if(scoring.epochHaveTag(i,pw_string)){jump_epoch(i); break;}
	QApplication::processEvents();
	
}

void UI_Mainwindow::goNextOut()
{
	int i = currentEpoch() + 1;
	for(;i< epoch_count; i++) 
		if(scoring.outliers[i]){jump_epoch(i); break;}
		QApplication::processEvents();
}


void UI_Mainwindow::goNextScoring(int score)
{
	int i = currentEpoch() + 1;
	for(;i< epoch_count; i++)
	{
		if(scoring.scoring[i] == score)	{ jump_epoch(i); break; }
	}
	QApplication::processEvents();
}


void UI_Mainwindow::autoScoring()
{
...
char* UI_Mainwindow::getViewBuf(int time_offset)
{
	char* viewbuf_cust;
	...
	for(i=0; i<signalcomps; i++)
	{
		...
		if(signalcomp[i]->ravg_filter_cnt)
		{
			...
			for(k=0; k<signalcomp[i]->ravg_filter_cnt; k++)
			{
				if(time < ((double)(signalcomp[i]->ravg_filter[k]->size + 3) / ((double)signalcomp[i]->edfhdr->edfparam[signalcomp[i]->edfsignal[0]].smp_per_record / signalcomp[i]->edfhdr->data_record_duration)))
				{
					time = (double)(signalcomp[i]->ravg_filter[k]->size + 3) / ((double)signalcomp[i]->edfhdr->edfparam[signalcomp[i]->edfsignal[0]].smp_per_record / signalcomp[i]->edfhdr->data_record_duration);
				}
			}
		}
	...
	if(hasprefilter)
	{
		for(i=0; i<signalcomps; i++)
		{
			if((signalcomp[i]->filter_cnt) || (signalcomp[i]->ravg_filter_cnt) || (signalcomp[i]->fidfilter_cnt) || (signalcomp[i]->ecg_filter != NULL))
			{
				signalcomp[i]->edfhdr->prefiltertime = (long long)(time * ((double)TIME_DIMENSION));
				if(signalcomp[i]->edfhdr->prefiltertime > viewtime)
				{
					signalcomp[i]->edfhdr->prefiltertime = viewtime;
					if(signalcomp[i]->edfhdr->prefiltertime<0) signalcomp[i]->edfhdr->prefiltertime = 0;
				}
			}
		}
		...
		for(i=0; i<signalcomps; i++)
		{
			if(signalcomp[i]->edfhdr->prefiltertime)  signalcomp[i]->records_in_viewbuf = (viewtime / signalcomp[i]->edfhdr->long_data_record_duration) - ((viewtime - signalcomp[i]->edfhdr->prefiltertime) / signalcomp[i]->edfhdr->long_data_record_duration) + 1;
			else signalcomp[i]->records_in_viewbuf = 0;
			...
			if(signalcomp[i]->edfhdr->prefiltertime)
			{
				...
				signalcomp[i]->samples_in_prefilterbuf
					+= (int)(((double)(viewtime % signalcomp[i]->edfhdr->long_data_record_duration)
					/ (double)signalcomp[i]->edfhdr->long_data_record_duration)
					* (double)signalcomp[i]->edfhdr->edfparam[signalcomp[i]->edfsignal[0]].smp_per_record);
			}
			else
		...
		viewbuf_cust = (char *)malloc(totalsize);
		if(viewbuf_cust==NULL)
		{
			live_stream_active = 0;
			QMessageBox messagewindow(QMessageBox::Critical, "Error", "Internal error: Memory allocation error:\n\"prefilterbuf\"");
		...
		for(i=0; i<signalcomps; i++)
		{
			if(!i)
			{
				datarecords = (viewtime - signalcomp[i]->edfhdr->prefiltertime) / signalcomp[i]->edfhdr->long_data_record_duration;
				...
				if((signalcomp[i]->viewbufsize>0)&&(datarecords<signalcomp[i]->edfhdr->datarecords))
				{
					...
					if(fread(viewbuf_cust + signalcomp[i]->viewbufoffset, signalcomp[i]->viewbufsize, 1, signalcomp[i]->edfhdr->file_hdl)!=1)
					{
						live_stream_active = 0;
						QMessageBox messagewindow(QMessageBox::Critical, "Error", "A read error occurred. 2");
						messagewindow.exec();
		...
		for(i=0; i<signalcomps; i++)
		{
			if((!signalcomp[i]->filter_cnt) && (!signalcomp[i]->ravg_filter_cnt) && (!signalcomp[i]->fidfilter_cnt) && (signalcomp[i]->ecg_filter == NULL)) continue;
	...
	for(i=0; i<signalcomps; i++)
	{
		if(viewtime>=0)  signalcomp[i]->records_in_viewbuf = ((pagetime + (viewtime % signalcomp[i]->edfhdr->long_data_record_duration)) / signalcomp[i]->edfhdr->long_data_record_duration) + 1;
		else  signalcomp[i]->records_in_viewbuf = ((pagetime + ((-(viewtime)) % signalcomp[i]->edfhdr->long_data_record_duration)) / signalcomp[i]->edfhdr->long_data_record_duration) + 1;
		...
		if(viewtime>=0)
		{
			signalcomp[i]->sample_timeoffset_part = ((double)(viewtime % signalcomp[i]->edfhdr->long_data_record_duration) / (double)signalcomp[i]->edfhdr->long_data_record_duration) * (double)signalcomp[i]->edfhdr->edfparam[signalcomp[i]->edfsignal[0]].smp_per_record;
			signalcomp[i]->sample_timeoffset = (int)(signalcomp[i]->sample_timeoffset_part);
			signalcomp[i]->sample_timeoffset_part -= signalcomp[i]->sample_timeoffset;
		}
		else
	...
	if(totalsize)
	{
		...
		if(viewbuf_cust==NULL)
		{
			live_stream_active = 0;
			QMessageBox messagewindow(QMessageBox::Critical, "Error", "The system was not able to provide enough resources (memory) to perform the requested action.\n"
				"Decrease the timescale and/or the number of signals on the screen and retry.");
			messagewindow.exec();
	...
	for(i=0; i<signalcomps; i++)
	{
		if(!i)
		{
			if(viewtime>=0)
			{
				datarecords = viewtime / signalcomp[i]->edfhdr->long_data_record_duration;
			}
			else
			{
				datarecords = 0;
			...
			if(dif<=0)
			{
				memset(viewbuf_cust + signalcomp[i]->viewbufoffset, 0, signalcomp[i]->records_in_viewbuf * signalcomp[i]->edfhdr->recordsize);
			...
			else
			{
				if(dif<signalcomp[i]->records_in_viewbuf)
				{
					...
					memset(viewbuf_cust + signalcomp[i]->viewbufoffset + readsize, 0, (signalcomp[i]->records_in_viewbuf * signalcomp[i]->edfhdr->recordsize) - readsize);
				...
				if(fread(viewbuf_cust + signalcomp[i]->viewbufoffset, readsize, 1, signalcomp[i]->edfhdr->file_hdl)!=1)
				{
					live_stream_active = 0;
					QMessageBox messagewindow(QMessageBox::Critical, "Error", "A read error occurred. 5");
					messagewindow.exec();
...
void UI_Mainwindow::create_epoch_file()
{
	scoring.init(this);
	refresh_scoring_array();

	for (int i = 0; i < epoch_count; i++)
	{
		annotate(na_string, i);
	}
	save_scoring();
	pagetime = TIME_DIMENSION * epoch_duration;
	maincurve->update();

}

void UI_Mainwindow::set_epoch_markers()
{
	bool ok;
	epoch_duration = QInputDialog::getInt(this, tr("QInputDialog::getInteger()"),
		tr("Epoch duration (sec):"), epoch_duration, 0, 1000, 1, &ok);
	...
	if(annotations_dock[0]==NULL)
	{
		annotations_dock[0] = new UI_Annotationswindow(0, this);
		addDockWidget(Qt::RightDockWidgetArea, annotations_dock[0]->docklist, Qt::Vertical);
		annotations_dock[0]->docklist->hide();
	}
	refresh_scoring_array();
	for (int i = 0; i < epoch_count; i++)
	{
		...
		annotation->onset = i*epoch_duration*TIME_DIMENSION;
		strncpy(annotation->annotation, na_string, MAX_ANNOTATION_LEN);
		annotation->annotation[MAX_ANNOTATION_LEN] = 0;
		edfplus_annotation_add_item(&annotationlist[0], annotation);
	}
...
void UI_Mainwindow::convert_motion_signal()
{
	std::ofstream qu;
	qu.open("c:\moem_log.txt", std::ios::out | std::ios::app);

	QString fileName = QFileDialog::getOpenFileName(this, tr("Open File"),
		"",tr("Files (*.txt)"));
	const char* fname = fileName.toStdString().c_str();
	
	// workaround. for some reason it is needed
	char buf[1024];
	strcpy(buf, fname);
	
	QString outfileName = QFileDialog::getSaveFileName(this, tr("Save File"),
		QFileInfo(fileName).path(), tr("Files (*.edf)"));
	const char* oofname = outfileName.toStdString().c_str();

	qu << "converting " << fname << " to " << oofname << "\n"; 
	convertMotion2edf(buf, oofname);
}


void UI_Mainwindow::score_rem()
{
	scoring.log("markers coumt", maincurve->active_markers->count);
	if(maincurve->active_markers->count == 0) 
	{
		QMessageBox messagewindow(QMessageBox::Warning, "8-|", "active_markers->count = 0");
		messagewindow.exec();
	}
	if(maincurve->current_annotation == NULL) 
		{
				QMessageBox messagewindow(QMessageBox::Warning, "8-|", "maincurve->current_annotation = 0");
				messagewindow.exec();
			return;
	}
	scoring.scoring[currentEpoch()] = s_REM;
	if(expertMode) scoring.expert[currentEpoch()] = true;
...
void UI_Mainwindow::score_nrem()
{
	if(maincurve->current_annotation == NULL) return;
	scoring.scoring[currentEpoch()] = s_SW;
	if(expertMode) scoring.expert[currentEpoch()] = true;
	strcpy(maincurve->current_annotation->annotation, scoring.epochName(currentEpoch()).c_str());
	annotations_edited = 1;
	setup_viewbuf();
...
{
	if(maincurve->current_annotation == NULL) return;
	scoring.scoring[currentEpoch()] = s_NA;
	if(expertMode) scoring.expert[currentEpoch()] = true;
	strcpy(maincurve->current_annotation->annotation, scoring.epochName(currentEpoch()).c_str());
	annotations_edited = 1;
	setup_viewbuf();
...
void UI_Mainwindow::score_wake()
{
	if(maincurve->current_annotation == NULL) return;
	scoring.scoring[currentEpoch()] = (byte)s_Wake;
	if(expertMode) scoring.expert[currentEpoch()] = true;
	strcpy(maincurve->current_annotation->annotation, scoring.epochName(currentEpoch()).c_str());
	annotations_edited = 1;
	// for motion scroring, 2 calculate mean wake motion

	float mrs = motion_mrs();
	sum_wake_mot_mrs += mrs;
	sum_wake_mot_n++;
...
void UI_Mainwindow::score_question() // 2 die
{
	if(maincurve->current_annotation == NULL) return;
	TStage s = (TStage) scoring.scoring[currentEpoch()]; // make this with an atribute of ? (bool)!
	if(s == s_Wake) s = s_WakeQuest;
	if(s == s_SW) s = s_SWQuest;
	if(s == s_REM) s = s_REMQuest;
	char buf[256];
	sprintf(buf, "%s??", maincurve->current_annotation->annotation);
...
{
	if(maincurve->current_annotation == NULL) return;
	scoring.switchTag(currentEpoch(),pw_string);
	//if(expertMode) scoring.expert[currentEpoch()] = true;
	strcpy(maincurve->current_annotation->annotation, scoring.epochName(currentEpoch()).c_str());
	annotations_edited = 1;		
	setup_viewbuf();
...
void UI_Mainwindow::refresh_scoring_array()
{
...
void UI_Mainwindow::score_by_motion()
{
	if(motion_signal < 0) 
	{
		QMessageBox msgBox;
		msgBox.setText("Select motion signal first");
		msgBox.exec();
		return;
	}
	...
	hours = QInputDialog::getInt(this, tr("Scoring by motion"),
		tr("Hours (0 - all):"), hours, 0, 1000, 1, &ok);
	int max_epoch = hours != 0 ? currentEpoch() +  hours*3600/epoch_duration : epoch_count - 1;
	if (max_epoch > epoch_count) max_epoch = epoch_count - 1;
	...
	for(int i = currentEpoch(); i < max_epoch; i++)
	{
		...
		float mrs = (signalcomp[motion_signal]->stat_sum_rectified / signalcomp[motion_signal]->stat_cnt) *
			signalcomp[motion_signal]->edfhdr->edfparam[signalcomp[motion_signal]->edfsignal[0]].bitvalue;
		if(mrs > thresh) { score_wake(); } else { shift_page_one_epoch(); }
		QApplication::processEvents();
	}
	if(annotations_dock[0])
		annotations_dock[0]->updateList();
	annotations_edited = 1;
}

void UI_Mainwindow::save_spectrum()
{
	// user should open spectrum view first and select the signal
	if(!spectrumdock || !spectrumdock->dock->isVisible()) 
	{
		QMessageBox msgBox;
		msgBox.setText("Open the spectrum window first");
	...
	bool ok;
	int hours = QInputDialog::getInt(this, tr("QInputDialog::getInteger()"),
		tr("Hours (0 for all recording):"), hours, 0, 1000, 1, &ok);
	int max_epoch;
	if(hours == 0) max_epoch = epoch_count;
	else
	{
		max_epoch = currentEpoch() + hours*3600/epoch_duration;
		if (max_epoch > epoch_count) max_epoch = epoch_count;
	}
	...
	// select file
	char dir[256];
	get_directory_from_path(dir, edfheaderlist[0]->filename,  256);

	QString fileName = QFileDialog::getSaveFileName(this, tr("Save File"), dir);
	if( fileName.isEmpty() || fileName.isNull() ) { return; }

	//if(!dirs.isNull() && !dirs.isEmpty()) chdir(dirs.toLocal8Bit().data());

	FILE *outputfile_all, *hourmotionfile; 

	outputfile_all = fopen(fileName.toLocal8Bit().data(), "w");
	char sfname[1024];
	sprintf(sfname, "%s.hourMotion.txt", edfheaderlist[0]->filename);
	hourmotionfile = fopen(sfname, "w");
	fprintf(hourmotionfile, "hour\tmotion\n");
	// определить сколько будет страниц и организовать цикл соотв-но
	int i = currentEpoch();

	char header[3000];
	strcpy(header, "scoring");
	get_current_spectrum(freqstep);
	int frmax = spectrum_max_freq/freqstep; 
	
	// make header

	float freq = 0;
	int fr;
	for(fr=0; fr<frmax; fr++)
	{
		sprintf(header, "%s\t%.1f Hz", header, freq);
		freq += freqstep;
	}
	sprintf(header, "%s\tsignal\tmotion\n", header, fr);

	fprintf(outputfile_all, header);
	// write data 
	float mean_hour_motion = 0;
	int hour_count = 0;
	int prev_hour = -1;
	for(;i < max_epoch - 1; i++)
	{
		FILE *outputfile; 
		char str[1024];
		data = get_current_spectrum(freqstep);
		if(data == 0) return;

		if(outputfile_all == NULL) {	fclose(outputfile); return;	}
		// get current epoch annotation (score)
		 
		char* title = maincurve->current_annotation->annotation;
		fprintf(outputfile_all, "%s\t", title);

		for(int fr=0; fr<frmax; fr++)
		{
			sprintf(str, "%.3f\t", data[fr]);
			remove_trailing_zeros(str);
			fprintf(outputfile_all, "%s", str);
		}

		fprintf(outputfile_all, "%.3f\t", signal_mrs(spectrum_signal));
		fprintf(outputfile_all, "%.3f\t", motion_mrs());
		fprintf(outputfile_all, "\n");
		
		// mean by hour
		int sel_viewtime = 0;
		long long l_tmp = edfheaderlist[sel_viewtime]->l_starttime + 
			edfheaderlist[sel_viewtime]->viewtime + 
			edfheaderlist[sel_viewtime]->starttime_offset;
		int hour = (int)((l_tmp / TIME_DIMENSION)/ 3600LL);
		if (hour >= 24) hour = hour%24;
		if(prev_hour > 0 && hour != prev_hour)
		{
			fprintf(hourmotionfile, "%d\t%.3f\n",prev_hour, mean_hour_motion / hour_count);
			hour_count = 0; mean_hour_motion = 0;
			prev_hour = hour;
		} else { prev_hour = hour; }
		mean_hour_motion += motion_mrs();
		hour_count++;		
		next_page();
		QApplication::processEvents();
	}	
	fclose(outputfile_all);
	fclose(hourmotionfile);
}
...
void UI_Mainwindow::autoScoreOutliers()
{
	...
	int signal = 0;
	if(spectrum_signal < 0){
		UI_SignalChooser signal_chooser_dialog(this, IOUTLIERSIGNALTASK);
		signal = selected_signal;
	} else signal = spectrum_signal;
	//IOUTLIERSIGNALTASK
	vector<float> MRS;
	MRS.resize(epoch_count);
	for(int i = 0; i < epoch_count; i++)
	{
	...
	vector<float> MRSs(MRS);
	sort(MRSs.begin(), MRSs.end());

	float med = MRSs[epoch_count/2];
	float p75 = MRSs[(epoch_count*3)/4];
	float p25 = MRSs[epoch_count/4];
	float max_val = med + 2*(p75-p25); // WARNING: sci-fi value
	for(int i = 0; i < epoch_count; i++)
	{
		if(MRS[i] > max_val){
			scoring.outliers[i] = true;
		}
	}
	// WARNING! 2do without saving
	save_scoring();
	load_scoring();
	jump_epoch(current);
...
{
	int e = (int)((edfheaderlist[0]->viewtime/TIME_DIMENSION)/epoch_duration);
	if(e < epoch_count) return e;
	return epoch_count;
}

void UI_Mainwindow::change_expert_mode(int forceExpert)
{
	if(forceExpert) expertMode = true; 
	else expertMode = !expertMode;
	if(expertMode)
	{
		setWindowTitle(tr("Moem ~ expert mode"));
		setWindowModified(true);
		//QMessageBox messagewindow(QMessageBox::Warning, "8-|", "Expert mode is on");
		//messagewindow.exec();
	}
	else 
	{
		setWindowTitle(PROGRAM_NAME);
		setWindowModified(true);
		//QMessageBox messagewindow(QMessageBox::Warning, ":-)", "Expert mode is off");
		//messagewindow.exec();
	}

}

void UI_Mainwindow::load_scoring()
{
	...
	edfplus_annotation_delete_list(&annotationlist[0]);
	scoring.loadScoring(sfname);
	sprintf(sfname, "%s.pw", edfheaderlist[0]->filename);
	scoring.loadPWScoring(sfname);

	if(epoch_duration >= 5) pagetime = TIME_DIMENSION * epoch_duration;
	maincurve->update();
	setup_viewbuf();
}


void UI_Mainwindow::save_scoring()
{
	if(files_open < 1 || epoch_count < 1) return;
	char fname[1024], sfname[1024];
	sprintf(sfname, "%s.slp", edfheaderlist[0]->filename);
	scoring.saveScoring(sfname);
	sprintf(sfname, "%s.pw", edfheaderlist[0]->filename);
	scoring.savePWScoring(sfname);
}

void UI_Mainwindow::delete_scoring()
{
	scoring.init(this);
	edfplus_annotation_delete_list(&annotationlist[0]);
	setup_viewbuf();
...
{
	UI_SignalChooser signalchooser(this, IMOTIONSIGNALTASK);
	sum_wake_mot_mrs = 0;
	sum_wake_mot_n = 0;
}

void UI_Mainwindow::report()
{

	if(!edfheaderlist[0])
	{
		QMessageBox msgBox;
		msgBox.setText("No file is opened...");
		msgBox.exec();
		return;
	}
	int hour_count = 3600/epoch_duration; // epochs in an hour
	if(3600%epoch_duration != 0)
	{
		QMessageBox msgBox;
		msgBox.setText("epoch duration is bad: an hour should be multiple of epoch duration");
		msgBox.exec();
	}
	bool ok;
	int hours = QInputDialog::getInt(this, tr("Report"),
		tr("Hours (0 - all):"), hours, 0, 1000, 1, &ok);
	...
	QString dirs = QFileDialog::getExistingDirectory(this, tr("report directory"), dir, QFileDialog::ShowDirsOnly
		| QFileDialog::DontResolveSymlinks);

	if(!dirs.isNull() && !dirs.isEmpty()) strcpy(dir, dirs.toLocal8Bit().data());


	char sfname[1024], sfname12[1024], sfname24[1024];
	sprintf(sfname, "%s/report_hour.txt", dir);
	sprintf(sfname12, "%s/report_12.txt", dir);
	sprintf(sfname24, "%s/report_24.txt", dir);
	f.open(sfname);
	f12.open(sfname12);
	f24.open(sfname24);
	...
	f << "start_epoch: " << i << endl;
	f << "Wake\tSW\tREM\n";
	
	double R = 0, R12= 0, R24= 0, S= 0, S12= 0, S24= 0, W =0, W12= 0, W24= 0;
	int n = 0;
	int n_hour = 0, n_12 = 0, n_24 = 0; // non outlier count
	for(; i < end_i; i++)
	{
		int stage = (int)scoring.scoring[i];
		...
		n++;
		if(!scoring.outliers[i]) { n_hour++; n_12++; n_24++; }
		if(n % hour_count == 0)
		{
			R12 += R; S12 += S; W12 += W;
			f << W/n_hour << "\t" << S/n_hour << "\t" << R/n_hour << endl; 
			R = 0; W = 0; S = 0; n_hour = 0;
		}
		if(n % (12*hour_count) == 0)
		{
			R24 += R12; S24 += S12; W24 += W12;
			f12 << n << "\t" << W12/n_12 << "\t" << S12/n_12 << "\t" << R12/n_12 << endl; 
			R12 = 0; W12 = 0; S12 = 0; n_12 = 0;
		}
		if(n % (24*hour_count) == 0)
		{
			f24 << W24/n_24 << "\t" << S24/n_24 << "\t" << R24/n_24 << endl; 
			R24 = 0; W24 = 0; S24 = 0; n_24 = 0;
		}
	}
...
void UI_Mainwindow::reportMotion()
{
	bool ok;
	int hours = QInputDialog::getInt(this, tr("QInputDialog::getInteger()"),
		tr("Hours (0 for all recording):"), hours, 0, 1000, 1, &ok);
	...
	FILE *hourmotionfile =  fopen(fileName.toLocal8Bit().data(), "w");
	fprintf(hourmotionfile, "hour\tmotion\n");
	int i = currentEpoch();
	// write data 
	float mean_hour_motion = 0;
...
float UI_Mainwindow::motion_mrs()
{
	// motion signal can be used from e.g. from video
	if (motion_signal < 0) return -1; 
	float s = signalcomp[motion_signal]->stat_sum;
	float mrs = (signalcomp[motion_signal]->stat_sum_rectified / signalcomp[motion_signal]->stat_cnt) *
		signalcomp[motion_signal]->edfhdr->edfparam[signalcomp[motion_signal]->edfsignal[0]].bitvalue;
	return mrs;
...
float UI_Mainwindow::signal_mrs(int signal)
{
	if (signal < 0) return 0;


	float s = signalcomp[signal]->stat_sum;
	float mrs = (signalcomp[signal]->stat_sum_rectified / signalcomp[signal]->stat_cnt) *
		signalcomp[signal]->edfhdr->edfparam[signalcomp[signal]->edfsignal[0]].bitvalue;
	return mrs;

}

epoch_stat UI_Mainwindow::getEpochStat()
{
	epoch_stat stat;
	if(motion_signal >= 0)
	{
		stat.motion_mrs = motion_mrs();
		stat.motion = true;
	} else stat.motion = false;
	...
	double* data = get_current_spectrum(freqstep);
	int imax = spectrum_max_freq/freqstep;
	stat.spectrum.resize(imax);
	for(int i=0; i<imax; i++)
	{
		stat.spectrum[i] = data[i];
	}
	return stat;
}

void UI_Mainwindow::add_current_epoch_stat(TStage astage)
{
	scoring.expert[currentEpoch()] = true;
	scoring.add_epoch_stat(getEpochStat(), astage);
}

double* UI_Mainwindow::get_current_spectrum(double& freqstep)
{
	int i, j, k;
	double 	dig_value;
	...
	//////////////////////////////////////////////////////////////////////////
	// index of chanel to be spectred

	signalcomp = this->signalcomp[spectrum_signal]; 
	if(signalcomp == NULL)
	{
		return NULL;
	}
...
void UI_Mainwindow::jump_epoch(int i)
{
	if (i>= epoch_count) return;
	edfheaderlist[sel_viewtime]->viewtime = epoch_duration*TIME_DIMENSION*i;
	setup_viewbuf();
}

void UI_Mainwindow::experize_epochs() 
{
	...
	for(int i = 0; i < epoch_count - 1 ; i++) //
	{
		...
		if(scoring.epochTags(i).size() > 0)
		{
			jump_epoch(i);
			QApplication::processEvents();
	
			vector<STag>::iterator it = scoring.epochTags(i).begin();
			while( it != scoring.epochTags(i).end())
			{
				scoring.add_epoch_tagStat(getEpochStat(), *it);
				it++;
			}
		}
	}
	...
	int bins = scoring.get_bins();
	
	// move 2 function

	QVector<double> w = QVector<double>::fromStdVector(scoring.mspectrum_w);
	QVector<double> sw = QVector<double>::fromStdVector(scoring.mspectrum_sw);
	QVector<double> r = QVector<double>::fromStdVector(scoring.mspectrum_rem);
	
	QVector<double> x(bins), w_10(bins), w_90(bins), sw_10(bins), sw_90(bins), r_10(bins), r_90(bins);
	QVector<double> pw(bins); // 2do add percentiles
	CleverScoring* cs = &scoring;
	bool is_pw = (cs->tagStatCount(pw_string) > 0);
	for (int i=0; i<bins; i++) 	
	{ 
		x[i] = i*freqstep; 
		if(scoring.mspectrum_wExist)
		{
			w_10[i] = cs->mspectrum_w[i] - cs->mspectrum_w_10[i]; 
			w_90[i] = cs->mspectrum_w_90[i] - cs->mspectrum_w[i]; 
		}
		if(scoring.mspectrum_swExist)
		{
			sw_10[i] = cs->mspectrum_sw[i] - cs->mspectrum_sw_10[i]; 
			sw_90[i] = cs->mspectrum_sw_90[i] - cs->mspectrum_sw[i];
		}
		if(scoring.mspectrum_remExist)
		{
			r_10[i] = cs->mspectrum_rem[i] - cs->mspectrum_rem_10[i]; 
			r_90[i] = cs->mspectrum_rem_90[i] - cs->mspectrum_rem[i];
		}
		if(is_pw) pw[i] = (cs->getTagStats(pw_string)).mspectrum[i];
	}
	...
	{
		customPlot->addGraph();
		customPlot->graph(graphId )->setErrorType(QCPGraph::etValue);
		customPlot->graph(graphId)->setDataValueError(x, w, w_10, w_90);
		//customPlot->graph(0)->setScatterStyle(myScatter);
		customPlot->graph(graphId)->setPen(QPen(Qt::black));
	...
	{
		customPlot->addGraph();
		customPlot->graph(graphId)->setPen(QPen(Qt::green));
		customPlot->graph(graphId)->setErrorType(QCPGraph::etValue);
		customPlot->graph(graphId)->setErrorPen(QPen(Qt::green));
		customPlot->graph(graphId)->setDataValueError(x, sw, sw_10, sw_90);
		customPlot->graph(graphId)->rescaleAxes(true, true);
	...
	{
		customPlot->addGraph();
		customPlot->graph(graphId)->setDataValueError(x, r, r_10, r_90);
		customPlot->graph(graphId)->setPen(QPen(Qt::blue));
		customPlot->graph(graphId)->setErrorType(QCPGraph::etValue);
		//customPlot->graph(2)->setScatterStyle(myScatter);
		customPlot->graph(graphId)->setErrorPen(QPen(Qt::blue));
		customPlot->graph(graphId)->rescaleAxes(true, true);
		graphId++;
	}
	if(is_pw)
	{
		customPlot->addGraph();
		customPlot->graph(graphId)->setData(x, pw);
		customPlot->graph(graphId)->setPen(QPen(Qt::red));
		customPlot->graph(graphId)->rescaleAxes(true, true);
		graphId++;
	}
	...
	qu->resize(500,500); 
	qu->exec();
	delete customPlot, qu, layout;
}

void UI_Mainwindow::annotate(const char* name, int epoch)
{
	struct annotationblock *annotlist, *annotation;
	//annotlist = annotationlist[0]; // of first file
	annotation = (struct annotationblock *)calloc(1, sizeof(struct annotationblock));
	annotation->onset = edfheaderlist[0]->starttime_offset + epoch_duration*epoch*TIME_DIMENSION;
	//annotation->former_annotation = annotlist;
	strncpy(annotation->annotation, name, MAX_ANNOTATION_LEN);
	annotation->annotation[MAX_ANNOTATION_LEN] = 0;
	edfplus_annotation_add_item(&annotationlist[0], annotation);
	
	/*if(annotlist == NULL)
		annotationlist[0] = annotation;
	else
		annotlist->next_annotation = annotation;
	annotlist = annotation;*/
}

void UI_Mainwindow::addNewTag(string name)
{
	scoring.addNewTag(name);
}