...
void convertMotion2edf(const char *fname, const char *ofname)

{
	...
	FILE* inputfile = fopeno(fname, "rb");
	// converting
	...
	char separator = ' ';
 	...
 	std::ofstream qu;
 	qu.open("c:\qu_log.txt", std::ios::out | std::ios::app);
	qu << "file: " << fname <<"\n";
	qu << "qu! " << m <<" "<< d<<" " << h <<" "<< mn << s << "\n";

	// test
	/*char qu[256];
	sprintf(qu, "%d %d %d", m, d, h);
	QMessageBox messagewindow_qu(QMessageBox::Critical, fname, qu);
	messagewindow_qu.exec();*/
	...
	if(edf_format)
	{
		sensitivity = 32767.0 / physmax;
	}
	else
	{
		sensitivity = 8388607.0 / physmax;
	}


/***************** write header *****************************************/
	strcpy(outputfilename, ofname); 
	...
	FILE* outputfile = fopeno(outputfilename, "wb");
	...
	p = snprintf(scratchpad, 128, "patient");
	for( ; p<80; p++)
	{
		scratchpad[p] = ' ';
	}
	...
	p = snprintf(scratchpad, 128, "recording");
	for( ; p<80; p++)
	{
		scratchpad[p] = ' ';
	}
	...
	int current_year =  tmData->tm_year + 1900; // test we will take current year!

	fprintf(outputfile,	"%02i.%02i.%02i%02i.%02i.%02i",d, m, current_year%100, h, mn, s);
	...
	while(1)
	{
		...
		if(line[i]==separator)
		{
			if(!column_end)
			{
				if(column == mot_column)
				{
					value = log(atof(line + str_start) + 1);
				}
				column_end = 1;
				column++;
			}
