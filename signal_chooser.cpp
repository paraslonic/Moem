...
void UI_SignalChooser::call_sidemenu(QListWidgetItem *)
{
  ...
  if(task == 1)
  {
	...
	mainwindow->spectrum_signal = list->currentRow(); //scoring
  }
  ...
  if(task == IMOTIONSIGNALTASK)
  {
	  mainwindow->motion_signal = list->currentRow();
	  QMessageBox messagewindow(QMessageBox::Information, "Ready", "motion signal selected");
	  messagewindow.setIconPixmap(QPixmap(":/images/ok.png"));
	  messagewindow.exec();
  }
  if(task == IOUTLIERSIGNALTASK)
  {
	  mainwindow->selected_signal = list->currentRow();
  }
}
